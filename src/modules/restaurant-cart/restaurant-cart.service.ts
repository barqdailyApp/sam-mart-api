...
export class RestaurantCartService {
  ...
  async getCartMeals() {
    ...
    const response = plainToInstance(GetCartMealsResponse, cart.restaurant_cart_meals.map((m) => {const total_unit_price=Number(m.meal.price)+Number(m.cart_meal_options.reduce((acc,curr)=>acc+curr.option.price,0));return {...m.meal,meal_id:m.meal.id,quantity:m.quantity,total_price:total_unit_price}}), {
