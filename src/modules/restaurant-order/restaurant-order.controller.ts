...
export class RestaurantOrderController {
  ...
  async getRestaurantOrdersDriverRequests(@Query() query: PaginatedRequest) {
    // add pagination
    const { orders, total } =
      await this.restaurantOrderService.getRestaurantOrdersDriverRequests(
        query,
      );
  ...
  @Roles(Role.DRIVER)
  @Get('driver-orders')
  async getRestaurantOrdersDriverOrders(
    @Query() query: GetDriverRestaurantOrdersQuery,
  ) {
    // add pagination
    const { orders, total } =
      await this.restaurantOrderService.getRestaurantOrdersDriverOrders(query);
  ...
  @Roles(Role.CLIENT)
  @Get('client-orders')
  async getRestaurantOrdersClientOrders(
    @Query() query: GetDriverRestaurantOrdersQuery,
  ) {
    // add pagination
    const { orders, total } =
      await this.restaurantOrderService.getRestaurantOrdersClientOrders(query);
  ...
  async driverAcceptOrder(@Param('id') id: string) {
    return new ActionResponse(
      await this.restaurantOrderService.driverAcceptOrder(id),
    );
  }
  ...
  async readyForPickup(@Param('id') id: string) {
    return new ActionResponse(
      await this.restaurantOrderService.pickupOrder(id),
    );
  }
  ...
  async cancelOrder(
    ...
    return new ActionResponse(
      await this.restaurantOrderService.cancelOrder(id, req),
    );
  }
  ...
  async addReview(
    ...
    return new ActionResponse(
      await this.restaurantOrderService.addReview(order_id, req),
    );
  }
  ...
  async addReviewReply(
    @Param('id') id: string,
    @Body() req: AddReviewReplyRequest,
  ) {
    return new ActionResponse(
      await this.restaurantOrderService.AddReviewReply(id, req),
    );
  }
  @ApiQuery({
    name: 'sort',
    enum: ReviewSort,
    required: false,
    description: 'Sorting criteria for reviews',
  })
  ...
  async getReviews(
    ...
    const { reviews, total } = await this.restaurantOrderService.getReviews(
      restaurant_id,
      query,
      sort,
    );
    const response = plainToInstance(RestaurantOrderReviewResponse, reviews, {
      excludeExtraneousValues: true,
    });
