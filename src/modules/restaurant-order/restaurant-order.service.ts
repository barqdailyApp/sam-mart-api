import { BadRequestException, Inject, Injectable, NotFoundException, Res, UnauthorizedException } from '@nestjs/common';
...
await this.notificationService.create(
  ...
  async getDriver(driver_id:string){
    ...
    async cancelOrder(order_id: string, req: CancelShipmentRequest) {
      ...
      const order = await this.restaurantOrderRepository.findOne({
        where: { id: order_id },
        relations:{user:true,payment_method:true,restaurant:true,address:true,driver:{user:true}}
      });
