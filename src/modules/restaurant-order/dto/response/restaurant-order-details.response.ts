...
export class RestaurantOrderDetailsResponse extends RestaurantOrderListResponse{ 
    ...
    @Expose()
    @Transform(({ obj }) => obj.restaurant_order_meals.map((meal) => plainToInstance(MealResponse,{...meal.meal,price:meal.price,id:meal.id,options:meal.restaurant_order_meal_options,total_price:meal.total_price},{excludeExtraneousValues:true})))
    meals:MealResponse[]
